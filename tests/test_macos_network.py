...
from core import common, netinfo  # noqa: F401
...
class MacNetworkHelpersTests(unittest.TestCase):
    def test_get_ssid_from_macos_networksetup(self):
        ...
        with patch.object(common, "IS_MACOS", True), patch.object(netinfo, "_run_decode", side_effect=run):
...
"""
        with patch.object(common, "IS_MACOS", True), patch.object(netinfo, "_run_decode", return_value=routes):
    ...
    def test_enabling_autostart_does_not_launch_second_instance(self):
        with tempfile.TemporaryDirectory() as temp_home, \
                patch.object(common, "IS_MACOS", True), \
    ...
    def test_gateway_mac_uses_macos_arp_format(self):
        ...
        with patch.object(netinfo, "_run_decode", return_value=arp), patch.object(netinfo, "get_gateway", return_value="192.168.50.1"):
