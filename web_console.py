...
口令比较用 hmac.compare_digest(抗时序侧信道); 控制台页面加载后会立刻把
URL 里的 key 从地址栏/浏览器历史中抹掉, 改用短时令牌(X-Console-Token)访问 API。
"""
import hashlib
import hmac
import json
...
# 短时令牌有效期(秒)。令牌 = HMAC(key, 时间片), 服务端无状态, 到期自然失效;
# 只经请求头传输, 不出现在 URL/浏览器历史里。
TOKEN_TTL = 3600
...
<div id="toast"></div>
<script>
const KEY_STORE = 'cnm_console_key';
let KEY = '', TOKEN = '';
(function initKey() {
  // 口令优先从 URL 取(扫码/复制链接)，取到后立刻从地址栏与浏览器历史里抹掉，
  // 改存 sessionStorage —— 否则口令会留在浏览历史、被肩窥或被随手分享出去。
  let k = new URLSearchParams(location.search).get('key') || '';
  if (k) {
...
function api(path, opts) {
  opts = opts || {};
  const h = Object.assign({'X-Console-Key': KEY}, opts.headers || {});
  if (TOKEN) h['X-Console-Token'] = TOKEN;   // 短时令牌: 只走请求头, 不进 URL
  opts.headers = h;
...
function ensureToken() {
  return api('/api/session', {method: 'POST'})
    .then(r => { if (r && r.token) TOKEN = r.token; })
    .catch(() => {});
}
function toast(msg) {
...
// 令牌有有效期: 定期换新, 保证长时间开着的控制台不会因令牌过期而失联
ensureToken().then(refresh);
setInterval(ensureToken, 1200000);
</script>
</body>
</html>
"""
...
<script>
var KEY_STORE = 'cnm_console_key';
function go(k) {
  try { sessionStorage.setItem(KEY_STORE, k); } catch (e) {}
  // 跳转 URL 不带口令: 否则口令会写进浏览器历史与地址栏(可被肩窥/随手分享)
...
function enter() {
  ...
  // 口令走 POST body 而非 query, 避免进入 URL / 历史 / 服务器访问日志
  fetch('/api/key', {method: 'POST', headers: {'Content-Type': 'application/json'},
                     body: JSON.stringify({key: k})}).then(function(r) {
    return r.json();
  }).then(function(j) {
    if (j && j.authed) {
      go(k);
...
// 自动预填: 优先 URL ?key=(兼容旧链接/扫码), 其次上次输入的口令
(function() {
  var url = new URL(location.href);
  var k = url.searchParams.get('key');
  if (!k) { try { k = sessionStorage.getItem(KEY_STORE) || ''; } catch (e) { k = ''; } }
  if (k) {
    ...
    def start(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def _authed(self, body=None):
                ...
                口令比较一律走 hmac.compare_digest —— 普通 `==` 会在首个不同字节处
                提前返回, 攻击者能通过响应耗时逐字节猜出口令(时序侧信道)。
                比较前统一 encode 成 bytes: compare_digest 对含非 ASCII 的 str 会抛
                TypeError, 而口令是外部输入, 不能让畸形输入把服务打成 500。
                """
                if self._token_valid(self.headers.get("X-Console-Token")):
                    return True
                candidate = self._key_from_request(body)
                if not candidate or not outer.key:
                    return False
                return hmac.compare_digest(str(candidate).encode("utf-8"),
                                           str(outer.key).encode("utf-8"))

            def _key_from_request(self, body=None):
                """口令来源: X-Console-Key 头 > query key= > POST body key=。"""
                header = self.headers.get("X-Console-Key")
                if header:
                    return header
            ...
            def _issue_token(self):
                """生成当前时间片的短时令牌(服务端无状态)。"""
                bucket = int(time.time() // TOKEN_TTL)
                return hmac.new(
                    str(outer.key or "").encode("utf-8"),
                    b"cnm-console-" + str(bucket).encode("ascii"),
                    hashlib.sha256).hexdigest()[:32]

            def _token_valid(self, token):
                """校验令牌; 同时接受上一时间片, 避免边界瞬间掉线。"""
                if not token or not outer.key:
                    return False
                ...
                for bucket in (now, now - 1):
                    expected = hmac.new(
                        str(outer.key).encode("utf-8"),
                        b"cnm-console-" + str(bucket).encode("ascii"),
                        hashlib.sha256).hexdigest()[:32].encode("ascii")
            ...
            def _send(self, code, body, ctype="text/html; charset=utf-8"):
                ...
                # 口令可能一度出现在 URL 里: 禁止把地址泄漏给第三方(Referer)
            ...
            def do_POST(self):
                ...
                # /api/key 与 /api/session 不走通用鉴权门: 它们本身就是"取口令/换令牌"的入口,
                # 且口令可以放在 POST body 里(避免出现在 URL / 浏览器历史)。
                if path == "/api/key":
                    self._json({"authed": self._authed(body)})
                    return
                if path == "/api/session":
                    if not self._authed(body):
                        self._json({"authed": False, "token": ""})
                        return
                    self._json({"authed": True, "token": self._issue_token(),
                                "ttl": TOKEN_TTL})
                    return
                if not self._authed(body):
                    self._send(200, KEY_ENTRY_HTML)
