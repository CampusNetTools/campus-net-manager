# 更新记录

## v5.5.0

「NAS 管家」：把路由器 NAS(alist) 的配置与日常使用全部搬进管家。

- **新增「NAS 管家」窗口**（路由器分区 → NAS 管家）：
  - 服务状态一览: alist / rclone / Tailscale 是否在跑、网页端是否可达、Tailscale IP
  - 服务控制: 启动 / 停止 / 重启（路由器侧幂等看门狗兜底，重启路由器后也会自动拉起）
  - 文件管理: 浏览 `/nas` 下各 USB 卷、上传、下载、删除、新建文件夹（大文件流式传输，不占大内存）
  - 一键打开 alist 网页端
- **凭据安全**: alist 登录口令用 Windows DPAPI 加密存 config.json（与 VPN 订阅同一机制），仓库不出现明文；换机器/换用户后口令自动失效需重填。
- **服务控制通道**: 新增路由器工作台转发端点 `router_assets/nas-api.sh`（与 proxy-api.sh 共用工作台令牌，仅放行 status/start/stop/restart 四个操作），不需要 SSH。
- 线程模型沿用 Clash 窗口固化的队列+泵模式，全部通过 `scripts/thread_safety.py` 静态检查。

## v5.4.4

路由器 Web 工作台补齐 + 公开仓库敏感值清理。这一轮的问题都不是"代码写错了"，而是**东西没在它该在的地方**。

- **补齐路由器 Web 工作台**：浏览器打开 `http://<路由器>:8088` 看到的页面，动作按钮从 8 个补到 11 个。缺的三个是「当前设备试用120秒」「关闭透明接管」「换一个可用节点」—— 后端 `action.sh` 一直支持，PC 端工作台也一直有按钮，**只有网页端没有入口**。另外新增「节点轮换」卡片（显示最近 3 次自动切换），并显示透明接管状态与 DNS 端口 7892。
- **确认框不再露出内部名**：以前点按钮弹的确认框写的是 `enable_transparent` 这类内部动作名，现在显示中文（与 PC 端工作台用同一套文案）。
- **`router_assets/console-index.html` 纳入版本控制**：它此前**只存在于路由器上**，仓库里没有 —— 既无法回滚、也无法部署，改坏了没有退路（这也是上面那个"页面漏了三个按钮却没人发现"的根因）。
- **清理公开仓库里的明文口令**：`HANDOFF.md` 与 `core/clash_api.py` 里写着一个真实令牌，而该令牌同时也是路由器管理密码、SSH 密码和家里 WiFi 密码 —— 等于把 WiFi 密码挂在公网上。已全部清除。⚠️ **强烈建议更换该口令**（WiFi 密码 + 路由器管理密码 + 工作台令牌），因为旧值已经公开过一段时间，清理当前版本并不能收回历史提交里的内容。
- `core/selfservice.py` 文档示例中的真实学号改为占位符（`config.json` 一直在 `.gitignore` 里，学号未泄露）。

### 新增的自动化卡口

- `scripts/console_page_lint.py` + `tests/test_console_page.py`：校验页面与 `action.sh` 的**动作名双向一致** —— 页面调的动作后端必须认识，后端有的动作页面必须有入口。同时检查标签配对、JS 语法（`node --check`）、元素 id 引用完整性。**双向**是关键：只查一个方向的话，正是这次的漏检点。
- `scripts/secret_scan.py` + `tests/test_secret_scan.py`：敏感值扫描。通用模式（关键词 + 具体值）进 CI；已知值模式从本地 `.secrets.local` 读词表。两个检查器都带**自检夹具** —— 先证明它抓得到已知问题，再接真实仓库，避免"静默报全过"。
- `scripts/deploy_router_assets.py`：路由资产部署工具纳入仓库，映射表补全为全部 12 个文件（原来只有 2 个，其余靠手工部署），并加上**覆盖前自动备份**（归档到 `/data/other_vol/proxy/backup/`）与 sha256 回读校验。凭据不硬编码，从 `--pwd` / `ROUTER_PWD` / `config.json` 取。
- CI 新增两道检查：路由器工作台页面检查、敏感值扫描。

### 发布过程中踩到并修掉的三个 CI 陷阱

v5.4.4 推上去后 CI 连红两次，而且**两次本地都是全绿** —— 这类"只在 CI 上红"的问题排查成本极高，所以连同修法一起记下来：

- **敏感值扫描漏扫未跟踪文件**：`secret_scan.py` 原来用 `git ls-files`（只列**已跟踪**文件）遍历仓库，于是"本地刚新建、还没 `git add`"的文件根本不进扫描 —— 而那恰恰是最可能夹带口令的时刻。本地新建的测试夹具含假值 → 本地绿；CI 是 checkout 出来的、所有文件都已跟踪 → 全部命中 → 红。已改为 `git ls-files --cached --others --exclude-standard`，并加"防回退"测试；测试夹具里的假值用行内 `# secret-scan:allow` 豁免。
- **Windows runner 的 stdout 默认是 cp1252**：脚本里一句 `print("检查通过")` 就会抛 `UnicodeEncodeError`，脚本以非 0 退出、CI 判该步骤失败。诡异之处在于**同一份逻辑在单测里是通过的**（unittest 不往 stdout 打中文），所以症状是"macOS 绿、只有 Windows 红"。已在 workflow 顶层设 `PYTHONIOENCODING: utf-8`，从源头消除这一类失败；同时给各独立脚本补上 `sys.stdout/stderr.reconfigure(encoding="utf-8")`（stderr 也要改 —— `sys.exit("中文提示")` 走的是 stderr）。
- **裸 `open().read()` 依赖 GC 关文件**：`console_page_lint.py` 与几个测试里都有，在 Windows 上会短暂锁住文件，且开启 `-W error::ResourceWarning` 时直接抛异常。已全部改为 `with`。

## v5.4.3

全仓库线程安全普查 + 下载测速口径修正。这一轮的多数问题只有真机才会暴露，静态检查与单元测试原本都是绿的。

- 新增 `scripts/thread_safety.py`：静态找出「**真的会在工作线程里执行**的 tkinter 调用」。tkinter 不是线程安全的，只有创建解释器的线程能安全碰控件。本机实测：mainloop 正在跑时从工作线程调 `widget.after` / `widget.configure` **不抛异常**（200 次全过），但没有 mainloop 或窗口已 `destroy()` 之后会抛 `RuntimeError: main thread is not in main loop` —— 也就是「有时能用、有时直接崩」。生产上表现为界面更新被静默丢掉，手测极难发现，所以做成 CI 卡口。
- 新增 `gui/ui_dispatch.py` 的 `post(widget, fn)`：工作线程 → 主线程的受保护投递入口，窗口已关或无事件循环时静默丢弃，**绝不把异常冒给工作线程**。
- 修复导出诊断报告时在工作线程里操作剪贴板：这两行以前被 `except` 静默吞掉，于是「复制失败」时提示语照样宣称「内容也已复制到剪贴板」，等于骗用户。现在把剪贴板操作移回主线程，并且只在真的复制成功时才那么说。
- 修复新手向导「检测副路由器」在工作线程里直接操作 Text 控件与按钮：探测本身仍在后台线程跑，界面更新改为投递回主线程。
- 修复下载测速未占用忙闲闸、可与批量测速叠跑导致结果互相覆盖；并修正先松闸再排队刷新的顺序，避免头部状态行卡在「正在下载测速…」不再恢复。
- 修复下载测速的标题会撒谎：测速走的是 mihomo「节点选择」的**当前出口**，与列表里高亮哪一行无关。以前标题直接写选中节点名，用户会把测得的带宽记到另一个节点头上。现在明确报当前出口，且当高亮节点与当前出口不一致时显式提醒「想测选中那个，请先切过去」。
- 修复探活失败（拿不到 HTTP 状态码）时把整行状态标红的问题：节点列表与 mihomo 版本其实都读到了，整行标红会让人误以为节点全挂了。现在单独写明「代理探活不可用（没连上 <主机>:7890）」，且不改变其它字段的颜色。
- 新增线程安全回归测试 `tests/test_thread_safety.py`：除了扫描全仓库，**还先用夹具自检检查器本身**——开发过程中它连续两次出现「静默报 0 处」（漏了嵌套 worker 函数、漏了 `Thread(target=self.method)` 这类类方法入口），以及把 `socket.bind`、`dict.update` 误判成控件调用。一个会静默失效的检查器比没有检查器更危险，所以两个方向都钉了断言。
- 新增 `tests/test_v543_fixes.py`：把本轮的每一处修复都钉成断言，防止悄悄回退。

## v5.4.2

- 新增「Clash 节点管理」窗口：列出节点选择组里的全部真实节点（名称/协议/延迟/使用中），支持筛选协议、搜索、测选中、全部测速、一键选最快、切换到选中、换一个可用节点、下载测速与更新订阅。
- 节点操作全部经路由器工作台新增的 `proxy-api.sh` 转发端点完成，电脑端不保存 mihomo 面板密钥、不使用 SSH；转发端点带令牌鉴权、接口白名单与「仅允许切换节点选择组」限制，且不接受外部测速地址，不会被当作通用探测跳板。
- 节点名走 base64 传输、接口路径走单层百分号编码，全程纯 ASCII，绕开 busybox 无法还原百分号转义的限制。
- 新增路由器侧节点轮换看门狗 `proxy-rotate.sh`：每 2 分钟检查显式代理是否真能连通（generate_204），失败时并行测出全部候选节点的真实延迟，按延迟升序做一次真实连通复验后落位，全不可用则还原原节点并写日志。
- 看门狗刻意「先测完再切、只切一次」：测速只打 `proxies/<name>/delay` 而不改当前选择，避免一次轮换在十几秒内把用户的连接改道多次；只按「能通」落位会被钉在慢速线路上（实测同一时刻 1279ms 与 212ms 并存），因此必须比过延迟。单目录锁 + 时间戳文件防并发，陈旧锁 5 分钟自动清理。
- 看门狗只操作 mihomo 控制 API 的节点选择，不碰透明代理、iptables 与中继配置；候选枚举自动剔除 Hysteria2（校园网封 UDP）与订阅夹带的信息行。
- 工作台新增「换一个可用节点」一键操作；状态接口新增 `proxy.rotate` 字段，工作台直接展示最近一条轮换日志。
- 订阅导入新增「启用协议」勾选项（VLESS / Hysteria2），部署时按勾选过滤节点，避免把整批必然失败的 UDP 节点写进配置；默认两种都保留，行为向后兼容。
- 修复 `relay_request` 在构造 opener 阶段抛出的网络异常未被包装为统一错误的问题，并按 `network` / `rejected` / `badjson` 分类，界面据此给出不同排查方向。
- 修复 v5.4.1 中订阅部署调用 `build_mihomo_config` 的一处语句粘连导致的语法错误。
- 修复工作线程直接调用 tkinter 的问题（真机上会抛 `RuntimeError: main thread is not in main loop`）：所有工作线程改为只向队列投递，主线程每 80ms 消费一次，并带窗口代际号防止旧窗口遗留的更新落到新窗口上。
- 修复节点表按延迟排序时数据源不一致：排序读的是 mihomo 的历史值、列表显示的是刚测出的值，导致点完「全部测速」再排序时顺序和数字对不上。
- 修复表头排序点击完全无效：排序意图被列表重填时的无条件重排覆盖。现在点同一列可切换升/降序，「未测」永远排在最后。
- 修复批量测速期间整表重填导致选中与滚动位置丢失，改为只重画测出结果的那一行。
- 修复「一键选最快」未占用忙闲闸、可与批量测速叠跑的问题。
- 修复测速、下载测速、切换节点完成后状态行卡在「正在…」不恢复的问题。
- 修复路由器不可达时批量测速显示成「测速完成 0/N 可用」的误导性提示（会被理解成节点全部失效）：现在明确报「测速中断」并指出是路由器连接问题；`api_delay` 不再吞掉环境层故障。
- 修复 mihomo 未运行时转发端点返回空正文、界面显示成「0 个节点」：端口无响应时明确返回错误并提示检查路由器上的代理进程。
- 协议筛选改为按订阅里实际存在的协议动态生成，出现 VMess/Trojan 时也能单独筛出。
- 下载测速增加墙钟上限与最小样本门槛，避免慢速管道上无限等待、或样本过小算出虚高带宽。
- 节点列表为空时给出部署与运行状态的排查引导。

## v5.4.1

- 修复小米 RD08 的透明转发：网页 TCP 流量改用 NAT REDIRECT，并用实时连接记录验证国内直连、国外代理；UDP 443 拒绝以触发浏览器回退 TCP，其余 UDP 保持直连。
- DNS 改为 redir-host 真实地址，配合 TLS/HTTP 域名识别，避免撤销接管后终端缓存虚拟 IP 而继续断网。
- 新增独立限时回退和实时故障守护，测试模式仅120秒，代理卡死实测约11秒恢复直连；规则安装失败立即清理。
- 工作台启用按钮明确为当前设备限时试用；关闭接管直接清理规则，不依赖代理配置可用。

## v5.4.0

- 新增 VPN 订阅安全导入：支持 Base64 URI 列表中的 VLESS / Hysteria2，校验后转换为 mihomo 配置；订阅地址、路由器令牌和面板密钥在 Windows 配置中使用 DPAPI 加密，安全导出时全部剔除。
- 新增路由器配置原子部署接口和 fail-open 透明代理脚本：先校验、备份、显式代理连通测试，再由用户单独启用透明接管；进程、配置或节点失败时自动撤销 TPROXY/DNS 规则并恢复直连。
- 路由器工作台操作改为 POST 请求体令牌，避免控制令牌进入 URL、浏览器历史和代理日志；新增启用/关闭透明接管按钮。
- 小米中继 Wi-Fi 档案改为普通网络监测模式，避免电脑端守护与路由器抢占校园网认证会话。
- 路由器局域网请求强制绕过 Windows 系统代理，避免失效的 VPN 本机代理让工作台误报离线；工作台改为显示透明接管实际开关，而不是仅凭监听端口猜测。
- 新增国内外分流规则：`.cn` 域名及中国大陆目标 IP 保持直连，其余流量才进入 VPN 节点；规则使用路由器本地 GeoIP 数据，不依赖运行时下载远程规则集。
- 修复 fail-open 健康检查把上游 HTTP 503 误判为成功的问题：启动、守护和配置部署现在都必须收到准确的 HTTP 204，否则立即撤销透明规则并恢复直连。

## v5.3.0

- 新增「连接进度」窗口：断电重启后展示路由器启动、中继关联、校园网认证与外网连通四阶段，并可在阶段卡住时触发安全的重连或重登。
- 路由器后台工作台新增深度自检、守护日志健康度、时钟偏差、MLO 状态与代理面板入口。
- 新增路由器连接恢复纯函数与回归测试。

## v5.2.4

- **修复 Windows 系统托盘图标「网」字糊化**：用户反馈托盘里只看到一个绿点、中间的「网」字消失。根因：`_make_tray_icon` 用 PIL 默认位图字体（8x8 像素）渲染「网」字，64x64 上字勉强可见，但 Windows 托盘实际渲染 16x16~24x24，`LANCZOS` 缩放后字完全糊化成绿色填充。现在用 truetype 字体（`msyh.ttc` 微软雅黑 / `simhei.ttf` 黑体 等，36px）+ 4 向 2px 绿色描边（与底色同色，缩到 16x16 时描边会被部分吃掉但字形仍清晰）；truetype 不可用时降级到几何网状图形（4 根白色放射线 + 中心节点，不依赖字体）。`Image` 模式由 `RGB` 改为 `RGBA`（背景透明避免边缘色晕）。新增 `tests/test_tray_icon.py`（3 项：尺寸正确 / 绿底白字占比 / 缩到 32/24/16 后白色像素仍存在）。

## v5.2.3

- **修复自动更新 WinError 32**：「应用更新失败：[WinError 32] 另一个程序正在使用此文件」是 Windows Defender / PopService 扫描刚写入的 exe 时短持锁（0.5~2s）触发的，shutil.move 内置不重试直接抛错。现在 `updater.move_with_retry` 最多重试 6 次、间隔指数退避（0.5s/1s/2s/4s/8s，累计 ≤16s），完全覆盖 Defender 短持锁时长；非 WinError 32/33 的异常立即透传，不浪费重试。在「校园网连接管家_new.exe」move 之前先 `unlink_quietly` 清掉上轮残留的同名文件（上次更新中断留下的，避免 move 覆盖锁文件）。
- 升级器 fallback 路径：`unlink_quietly(path)` best-effort 删除（吞 OSError）；`_retry_on_lock` 装饰器（仅 Windows 生效）；`move_with_retry(src, dst)` 公开 API。新增 3 个测试（unlink_quietly、模拟短锁重试恢复、非锁异常透传），unittest 24 项全绿、ruff 干净。

## v5.2.2

- **修复「设备管理」在线设备列表拉取失败**：v5.2.0 猜的 Dr.COM 自助服务地址 `8080/Self/` 在立达学院并未部署（认证服务器只开 80 登录页 + 801 ePortal 门户），导致「刷新设备列表」所有请求连不上、被误报成"自助服务登录失败"。实测校准后改用 Dr.COM 哆点 ePortal 门户接口 `http://<认证主机>:801/eportal/portal/online_list`，仅凭 `user_account`（账号@运营商后缀）即可返回当前账号下所有在线设备，无需登录自助服务、甚至不需要密码。
- **设备名称自动识别，无需手输**：在线列表返回的 `dhcp_host` 字段就是设备真实名（如小米路由器显示 `MiWiFi-RD08`）。「保护设备」区域新增「自动识别」按钮，一键把当前出口设备（本机/路由器）的真实名回填到名称框；在线列表每台设备也新增「设为保护」按钮，点一下即回填该设备名。列表同时标记「本机/路由器」当前设备。
- **下线操作语义校准**：实测 ePortal 门户的 `logout` 接口只注销"请求源 IP"（本机/路由器）的会话，不支持按会话远程下线其他设备（传 `online_session` 参数被忽略）。因此「解除绑定」改为对当前设备执行「注销」（守护会自动重连），对其他设备明确提示"学校未开放用户侧远程下线接口"，不再误导用户。
- 重写 `core/selfservice.py`（JSONP 解析兼容 Dr.COM 实际返回的结尾分号 `callback({...});`）、更新 `gui/device_manager.py`、重写 `tests/test_v520_device_manager.py`（18 项测试全绿）。

## v5.2.1

- **Windows 软件名统一改中文「校园网连接管家」**：打包产物从英文 `CampusNetManager.exe` 改为 `校园网连接管家.exe`（与 macOS `.app` 一致），发布资产名同步为 `校园网连接管家-<版本>-win64.exe`，用户下载到的不再是英文文件名。
- **更新后只保留最新一个软件**：自动更新完成时，把新文件统一重命名为规范中文名 `校园网连接管家.exe`（无论之前跑的是英文名还是带版本号的旧名），并删除同目录下所有历史版本残留（`CampusNetManager*.exe`、`校园网连接管家-v*.exe` 等）。macOS 端同样清理旧英文名 `CampusNetManager.app`。旧的下划线命名（`CampusNetManager_v*_*`）仍作次选向后兼容，避免选到历史重复包。
- 新增 `updater.find_stale_executables` / `find_stale_apps` 枚举待清理旧版本；`windows_apply_script` 增加 `final_exe` 参数支持「重命名为规范中文名 + 清理旧 exe」。

## v5.2.0

- **新增「设备管理」窗口**（主窗导航「连接」分组新增入口），解决校园网单账号多设备的两个核心诉求：
  - **保护设备（固定一直在线、不被挤掉）**：可开启保护模式并为设备命名。开启后守护把会话刷新强化为「每轮循环都刷新」，让本机/路由器的出口会话始终是"最新"会话 —— 校园网按会话新鲜度淘汰设备，新设备登录时被挤掉的就是别人，而不是你指定的这台。底层复用并强化既有 `kick_guard` 机制，新增 `protected_device` 配置项（enabled/name/kind）。
  - **在线设备管理（一键解除绑定）**：拉取校园网自助服务系统（Dr.COM `/Self/`）当前账号的在线设备列表，展示设备/IP/MAC，点「解除绑定」即把占用名额的其他设备强制下线，无需再进浏览器自助系统手动操作。
- **新增 `core/selfservice.py` 自助服务客户端**：登录会话维护（Cookie）、在线设备列表拉取、强制下线/解除绑定。Dr.COM 自助服务不同学校部署差异大，端点与字段名均做成可覆盖常量 + 宽松解析（JSON / HTML 表格两种形态），设备联网后可按实际响应微调，不会因格式不符直接崩。
- 守护循环保护模式下刷新阈值从「每 3 轮」降为「每轮」，并记录明确日志（"保护设备「X」刷新会话"）。
- 新增 `tests/test_v520_device_manager.py` 覆盖：保护设备配置补齐、自助服务地址推导、设备列表 JSON/HTML 解析、登录/下线判据、无凭据优雅报错。

> 说明：自助服务接口需设备处于校园网环境才能实测；本版本实现基于 Dr.COM 自助服务标准结构 + 宽松解析，待设备回宿舍联网后做一轮实测校准即可。

## v5.1.2

- **配置层加固（上次审查遗留项全部落地）**
  - `ensure_lida_profile` 不再强改用户自建档案：旧实现会把任何名为「校园网」且未填 SSID 的档案无条件改名并绑定立达 SSID，用户在 v5 自建的"任意网络"档案会被静默改语义（绑定后不再匹配其他网络）。现在只迁移真正的旧版遗留种子档案，并用 `lida_migrated` 标记保证迁移只做一次，此后即使用户再建同名档案也不会被动。
  - **Windows 密码改用 DPAPI 加密存储**：此前 Windows 下密码以明文写在 `config.json`。现在用当前用户范围的 `CryptProtectData` 加密为 `password_enc`（`password_store: "dpapi"`），加载时自动解密；旧版明文配置首次加载时自动迁移。导出配置（`config_for_export`）不含密文与明文，可安全分享。
  - 修复「密码已加密后修改密码不落盘」：原判据会因 `password_store` 已是 dpapi 而跳过写盘；现在按明文是否变化决定是否重新加密。
- **认证与网络探测**
  - `check_auth` 增加第二判据：除「注销页」标题外，再匹配 Dr.COM 注销页特征（`注销`/`logout` 等），降低门户改版导致「已登录」被误判为未登录的概率。
  - `auth_reachable` 增加防抖：守护循环的环境判定连续 3 次失败才翻转，避免单次抖动触发误切档案；同时提供 `debounce=False` 给"此刻必须看真相"的调用点（重登失败后判链路、界面展示、自动切档案），已把守护循环/匹配/控制台/隧道引导四处改为实时探测。
  - `ensure_login` 改为指数退避（2s 起、封顶 10s），并在重试前先探一次认证服务器：明确不可达时只做 2 次快速重试即收工，不再死等 20 秒×10 次。
- **安全**
  - Web 控制台口令比较改用 `hmac.compare_digest`（防时序侧信道）；口令不再出现在 URL 查询串（避免进浏览器历史/被代理日志留存），改为 `X-Console-Key` 头 + 短时 token 校验。
  - Web 控制台 `/api/status` 增加 TTL 快照缓存，避免多设备并发轮询时反复触发 PowerShell 采集。
- **更新器**
  - 自动更新下载增加 **SHA256 校验**：优先取 Release 的 `.sha256` 校验和资产，缺失时才回退到 GitHub API 的 `digest` 字段；不匹配则拒装并提示。新增 `scripts/make_checksum.py` 生成校验和，CI 发布时自动附带 `*.sha256`。
  - 资产挑选改为按平台精确匹配（macOS 取 `*macos*.zip`、Windows 取 `*win64*.exe`），避免 Release 里多个包时挑错。
- **竞态与单实例**
  - `core/matching.py` 与 `core/daemon.py` 遍历档案前先取快照（浅拷贝），消除守护线程自动切档案与 GUI 保存并发的遍历异常。
  - 单实例锁增加**进程启动时间指纹**：原先只比对 PID，PID 被系统复用时会误判"已有实例在跑"而拒绝启动；现在记录 `pid + 启动时间戳`，指纹不符即视为陈旧锁并接管（兼容旧格式锁文件）。
- **静态检查抓到的真实缺陷（7 处）**
  - 修复 `except ... as exc` 后把 `exc` 捕获进延迟执行的 `lambda` —— Python 会在 `except` 块结束时删除该变量，导致这些**错误提示回调本身抛 `NameError`**（用户看到的是"回调崩溃"而非真实错误）。涉及 `gui/daemon_ctl.py`、`gui/router_console_ui.py`(×3)、`gui/router_tools.py`、`gui/update_ui.py`。
  - 清理 `gui/wizard.py` 重复导入、`scripts/v5_driver_check.py` 无效占位常量、测试中的未用变量。
  - CI 静态检查由 pyflakes 换成 ruff 高信号规则（`F` + `E9`），并**从"只报告"改为"不通过即失败"**（`F403/F405` 因项目 star-import 架构属已知误报，显式忽略）。
- **清理**：删除冗余脚本 `lida_keepalive.py`（166 行，与 `keepalive_core.py` 重复且无任何引用）；删除 `core/sysutils.py` 中错误的 `AUTOSTART_CMD` 常量（非打包态指向 `core/sysutils.py` 而非入口脚本，一旦被采用会写出错误的自启命令），并在 `scripts/split_core.py` 归属表中同步移除。
- 新增 `tests/test_v512_fixes.py` 覆盖上述修复（DPAPI 往返、一次性迁移、口令时序比较、SHA256 校验、进程指纹锁、校验和脚本等）。

## v5.1.1

- **修复 Windows 下 `get_lan_ips()` 恒返回空的问题**：IP 枚举原本只支持 `ifconfig`（macOS），导致隧道共享引导页显示"本机IP"、网络控制台二维码指向 `127.0.0.1`（手机扫不开）。现在 Windows 走 PowerShell `Get-NetIPAddress` + `ipconfig` + `getaddrinfo` 兜底，并过滤 VPN 虚拟接口（接口名含 VPN / TUN / 198.18.x Clash 假 IP）。
- **修复 Windows 自更新中文路径必失败**：bat 脚本原以 UTF-8 写盘而 cmd 按 ANSI（GBK）解码，`桌面\校园网连接管家.exe` 会变乱码导致替换失败。现在按 Win32 `GetACP()` 的真实代码页写盘；`find`/`timeout` 改用 System32 绝对路径，避免被 PATH 上的 GNU 工具（Git Bash 等）劫持。已用中文路径端到端实测替换成功。
- **配置文件读写加固**：`save_config` 改为临时文件 + `os.replace` 原子写入，强杀进程/断电不再产生半截 JSON；`load_config` 遇损坏文件自动备份为 `config.json.corrupt-<时间戳>` 并重建默认配置（此前会直接崩溃且无提示）；新增可重入读写锁，消除守护线程自动切档案与 GUI 保存的并发竞态。
- **修复「一键开启移动热点」永远失败**：旧实现调用了不存在的 `Start-MobileHotspot` cmdlet。改用 WinRT `NetworkOperatorTetheringManager`（轮询 15 秒确认状态）；失败时如实提示并自动打开「设置 → 移动热点」。
- **认证探测绕过系统代理**：`http_get` 兜底路径、captive portal 探针、诊断登录探测此前走系统代理（Clash 等），开着代理软件会把探测/登录（含账号密码）发往代理出口。现在统一绕过代理并绑定物理网卡，与 `try_login` 行为对齐。
- **修复 Windows 会话归属分析崩溃**：`local_macs()` 调用不存在的 `ifconfig`；改为 `ipconfig /all` 直接按 MAC 格式匹配（兼容中文/英文系统与 GBK 编码）。
- **修复 Windows VPN 误报**：`vpn_active()` 原以宽泛的 `"vpn"` 子串匹配 `route print` 输出（装过 VPN 软件即常驻误报）；收窄为 WireGuard/wintun/TAP/OpenVPN 等精确驱动名。
- **修复中文 Windows 网关兜底解析**：`route print` 的 On-link 在中文系统显示「在链路上」，兜底分支未校验 IP 会把它当网关返回；已加 IPv4 校验。
- **修复断网时间线解析崩溃**：历史行时间格式异常时 `analyze_outage_timeline` 会直接抛异常，现逐行保护跳过。
- 同步更新热点相关测试至新 WinRT 实现。

## v5.1.0

- 新增「路由器后台工作台」窗口：把部署在路由器上的监控工作台融合进管家界面，直接读取路由器控制台状态接口（无需 SSH），实时展示无线中继 / 本机 AP / 校园网认证 / 外网连通 / 透明代理 / VPN / SSH / 防复发开关 / 系统负载 / 守护日志。
- 工作台支持一键操作：重启透明代理、重启 VPN、重新登录校园网、重连中继、重启 WiFi、重启路由器（危险操作二次确认）。
- 工作台支持可视化切换中继目标（填 SSID + 密码即可换校园网），路由器地址 / 端口 / 令牌可在窗口内修改并保存。
- 修复 Windows 下探测校园网时闪现终端窗口的问题：`curl` 探测调用补上 `CREATE_NO_WINDOW`（Windows 11 默认终端为 Windows Terminal，未隐藏时会弹出终端框）。
- 界面整理：功能导航两列强制等宽（`uniform`），左侧操作按钮等宽，超长按钮文字缩短；工作台窗口按内容自适应尺寸，避免留白与裁切。
- 修复测试中的「时间炸弹」：时间线测试改用相对日期，避免固定日期过期后误报失败。

## v5.0.15

- 根据用户确认改用 iPhone 系统内建 IKEv2，摆脱中国大陆 App Store 中 WireGuard 客户端不可用的限制，不要求 iPhone App 或 Apple 签名。
- 新增 IKEv2 `.mobileconfig` 与 strongSwan `swanctl` 凭据片段生成核心：要求网关已授权、`swanctl` 已存在、CA 证书、服务器地址和证书标识齐全；任一缺失即拒绝生成。
- 描述文件含单机 EAP 密码和网关 CA，明确仅允许本地一次性交付；不包含校园网密码。v5.0.14 WireGuard 路线已留档。

## v5.0.14

- 根据用户选择，手机端改用官方免费 WireGuard App 扫描本项目生成的标准配置；不再要求自有 iOS App、Xcode 或 Apple Developer 签名。
- 新增 Mac 端 WireGuard 配对核心：仅当本地网关、`wg` 工具和服务端公钥均就绪时生成独立手机密钥、导入配置和 server peer 片段；未就绪时失败关闭。
- 官方 App 的标准二维码含设备私钥，改为一次性本地显示约束，禁止写入日志、配置或聊天内容。保留 v5.0.13 自有 iOS 基座完整快照以便日后恢复。

## v5.0.13

- 手机全流量共享路线切换为“自有校园网连接管家 + WireGuard 数据面”；官方 WireGuard Apple 源码以固定提交的 Git 子模块纳入，未复制或改名为自有代码。
- 新增第三方许可声明、WireGuard 架构与安全边界文档，以及上游提交和 MIT 许可证自动校验，防止依赖被静默替换或升级。
- 真机签名、WireGuardKit 接入和 Mac 特权网关仍需完整 Xcode、项目自己的 Apple Developer Team 与用户可见授权后实施。

## v5.0.12

- iPhone Packet Tunnel 在 TLS 系统证书校验之外，新增二维码携带的叶证书 SHA-256 指纹校验；指纹格式无效、证书不匹配或网关拒绝令牌时均拒绝连接，不回退到明文。
- 手机端只有收到电脑网关对一次性令牌的 `ready` 确认后才完成 VPN 启动；认证中断不会再显示为“已通过电脑隧道连接”。
- 为本阶段源码留存 `archives/source-v5.0.11-before-mobile-auth` 可恢复快照；iPhone 真机签名、Mac 特权 TUN/NAT 网关仍未实施。

## v5.0.11

- TCP 转发改为有界缓冲的双向非阻塞循环；按双向整体活动判断空闲，避免持续单向传输被另一方向的计时器截断。
- 保留 HTTP Upgrade 握手，支持升级后的双向字节转发；普通 HTTP 仍单请求连接，HTTPS 不解密。
- 支持 IPv6 CONNECT 地址，区分本地服务连接失败、外网连接失败和传输空闲超时。
- 诊断分开显示当前连接地址数与历史授权记录数，不再把历史记录当在线设备。
- 补充长连接、空闲超时、升级双向交换、IPv6 回归；微信现场效果待用户验收。

## v5.0.10

- 修复经 HTTP 代理访问控制台时重复拼接口令参数导致鉴权失败。
- 本机控制台直接连接，不经过 VPN 上游；支持控制台 POST 请求转发。
- 用户较晚确认设备授权后仍保存允许记录，手机后续重试可用。
- HTTP 请求禁止跨目标长连接复用，过滤代理专用认证头；添加转发字节与最近错误诊断。
- 手机实际外网连通性及速度仍需现场复测，不以配置页可达作为验收。

## v5.0.9

- 隧道启动检查上游端口；无效的 7890 配置显示修复选择，不再把配置页可达误报为外网正常。
- 保存、关闭或预设 VPN 上游后，对正在运行的隧道新连接即时生效。
- 同一手机并发请求共享一次授权，避免重复弹窗和长时间排队。
- 修复上游 CONNECT 响应头后首包丢失；释放转发 socket、清理结束线程，停止共享时关闭连接。
- 新增“检查代理外网出口”；文案区分系统 VPN 与额外 HTTP 上游。
- 添加真实 socket 回归测试，隔离网关测试对现场网络的依赖。

## v5.0.8

- **修复「主窗隐藏时按钮全部失灵」(用户报告的按钮无效问题)**:
  - 根因: 手机连入隧道共享时弹出「设备连接请求」授权框(messagebox 系统级模态),
    主窗已缩到菜单栏时该框不可见, 却挡死整个应用——所有窗口的按钮/滚动全部无效。
  - 修复: `_poll_allow` 检测主窗隐藏时**不再弹模态框**, 改为自动拒绝 + 系统通知
    「打开主界面后让对方重连一次即可授权」。
- **全界面按钮级真实点击审计** (Quartz 合成鼠标逐个真点, 新增 `scripts/click_audit.py`):
  - 主窗 20 个按钮 + 各子窗全部通过; 顺带修掉 3 个审计暴露的问题:
    1. 路由器探测失败回调 NameError(except 作用域闭包引用已删除的 e);
    2. 路由器检测后台探测回来时窗口已销毁 → TclError(补 winfo_exists 防护);
    3. 新手向导「← 上一步」初始无命令却可点(改为进入子步骤才显示)。
  - 复测确认: 展开/使用帮助/保存设置(滚动到底)/生成PAC/生成分步路径/查询官方适配 全部响应正常。
- 「测试通知」按钮补充日志反馈。
- 全套 243 测试通过。

## v5.0.7## v5.0.8

- **修复「主窗隐藏时按钮全部失灵」(用户报告的按钮无效问题)**:
  - 根因: 手机连入隧道共享时弹出「设备连接请求」授权框(messagebox 系统级模态),
    主窗已缩到菜单栏时该框不可见, 却挡死整个应用——所有窗口的按钮/滚动全部无效。
  - 修复: `_poll_allow` 检测主窗隐藏时**不再弹模态框**, 改为自动拒绝 + 系统通知
    「打开主界面后让对方重连一次即可授权」。
- **全界面按钮级真实点击审计** (Quartz 合成鼠标逐个真点, 新增 `scripts/click_audit.py`):
  - 主窗 20 个按钮 + 各子窗全部通过; 顺带修掉 3 个审计暴露的问题:
    1. 路由器探测失败回调 NameError(except 作用域闭包引用已删除的 e);
    2. 路由器检测后台探测回来时窗口已销毁 → TclError(补 winfo_exists 防护);
    3. 新手向导「← 上一步」初始无命令却可点(改为进入子步骤才显示)。
  - 复测确认: 展开/使用帮助/保存设置(滚动到底)/生成PAC/生成分步路径/查询官方适配 全部响应正常。
- 「测试通知」按钮补充日志反馈。
- 全套 243 测试通过。

## v5.0.7## v5.0.7

- **修复「路由器代理等窗口看得见却点不动」(滑动/按钮全部失灵)**:
  - 根因: 隧道共享就绪窗口打开时会 `grab_set` 抢占全局事件(模态), 用户把它留在后台
    再去开其他功能窗口时, 新窗口收不到任何鼠标/滚轮事件——看得见摸不着。
  - 修复: 移除全部功能窗口的全局事件抓取(隧道就绪窗 + VPN 配置弹窗),
    功能窗口一律并列非模态; 新增 `tests/test_no_modal_grab.py` 静态扫描禁止回归。
  - 运行时验证: 开隧道窗 → grab_current=None; 保留隧道窗再开路由器代理 → 仍无 grab。
- 附带: 上一条 v5.0.6 的「免认证网络提示」与网卡类型判定保持不变。

## v5.0.6## v5.0.7

- **修复「路由器代理等窗口看得见却点不动」(滑动/按钮全部失灵)**:
  - 根因: 隧道共享就绪窗口打开时会 `grab_set` 抢占全局事件(模态), 用户把它留在后台
    再去开其他功能窗口时, 新窗口收不到任何鼠标/滚轮事件——看得见摸不着。
  - 修复: 移除全部功能窗口的全局事件抓取(隧道就绪窗 + VPN 配置弹窗),
    功能窗口一律并列非模态; 新增 `tests/test_no_modal_grab.py` 静态扫描禁止回归。
  - 运行时验证: 开隧道窗 → grab_current=None; 保留隧道窗再开路由器代理 → 仍无 grab。
- 附带: 上一条 v5.0.6 的「免认证网络提示」与网卡类型判定保持不变。

## v5.0.6## v5.0.6

- **有线/无线按网卡硬件类型判定 (修「Wi-Fi 被误判为有线以太网」)**:
  - 用户指出: 免认证校园 WiFi(有密码即可上网、不限设备)下被显示成「有线」。
  - 根因: macOS 26 隐私机制对无定位授权进程打码 SSID——`networksetup -getairportnetwork` 甚至谎报 "not associated", 旧逻辑「读不到 SSID = 有线」随之失真。
  - 修复: `get_connection_mode` 改按**默认路由网卡的硬件端口类型**判定 (networksetup 硬件端口 = Wi-Fi 且 `ipconfig getsummary` 存在 SSID 行[内容可是 <redacted>] → 无线; 以太网类 → 有线)。SSID 打码时环境行显示「WiFi 名被系统隐藏」并一次性日志引导开定位服务。
- **隧道共享窗口新增「免认证网络」提示**: 顶部固定提示先判断 WiFi 类型——免认证 WiFi(有密码即可上网/不限设备)手机直连同一 WiFi 即可, 无需隧道共享; 需网页认证的(如 LIDA-UNIVERSITY)手机过不了认证再走 ② 配代理。
- 新增 `tests/test_iface_connection_mode.py` (6 项) + 扩充网关模式判定测试, 全套 243 项通过。

## v5.0.5

- **修复「直连校园网被误判为经由路由器接入」**:
  - 现象: 电脑有线直连宿舍校园网端口(网关 10.14.0.1), 隧道共享却提示「经路由器接入, 手机连路由器 WiFi 不用配代理」, 用户困惑。
  - 根因: 旧启发式「网关是私网地址+有 MAC → 路由器」——但**校园网自身就是大内网**(立达 WiFi 网关 10.52.191.254 / 有线 10.14.0.1 均为私网), 该启发式在校园网下必然误判。
  - 修复: `detect_gateway_mode` 增加两个高区分度信号(向后兼容, 可选参数):
    1. SSID 命中校园网档案绑定 SSID → 判定「电脑直连校园网 WiFi」;
    2. 有线接入 + 校园认证可达 → 判定「电脑有线直连校园网(校园网自身是内网)」。
    其余场景维持旧启发式(自家路由器 WiFi/插路由器仍判 router, 家里华为中继场景不受影响)。
  - 真机验证: 有线 10.14.0.1 + 认证可达 → 判定 computer「电脑有线直连校园网」✓
  - 新增 `tests/test_gateway_mode_detect.py` (6 项), 全套 236 项通过。

## v5.0.4## v5.0.4

- **修复「关闭窗口即崩溃弹窗」(意外退出对话框)**:
  - 现象: 点 X 关闭窗口时弹出「校园网连接管家意外退出」, 崩溃报告自 v4 时代累计多份。
  - 根因(崩溃栈实锤): 关闭走「最小化到托盘」时, pystray 在**后台线程**运行 NSApplication, 新版 macOS 在 `NSUpdateCycleInitialize` 处 SIGTRAP(EXC_BREAKPOINT, Trace/BPT trap 5)。
  - 修复: 新增 `gui/tray_mac.py` — macOS 改用 **NSStatusItem 原生菜单栏托盘**(pyobjc, 在 Tk 主线程创建, 菜单动作经主线程事件循环派发, 线程安全); pystray 仅保留给 Windows。
  - 退出兜底: Tk destroy 后若 AppKit/线程残留偶发 SIGTRAP 假崩溃, 在守护/代理/控制台/配置全部清理完成后 `os._exit(0)` 干净退出, 不再弹「意外退出」。
  - 托盘交互不变: 关 X = 隐藏窗口守护继续, 菜单栏 🌐 图标 → 打开主界面 / 退出; 托盘创建失败时窗口保持显示不隐身。

## v5.0.3

- **真实点击测试打通 + 文案清理**:
  - 新增 Quartz 合成鼠标点击测试路径(走辅助功能授权, 不依赖「自动化」权限): 驱动窗口置顶 → 合成点击「隧道共享」→ 截屏验证窗口被点开, 完整模拟真人操作。
  - 清理隧道共享窗口 4 处 markdown 星号残留(`**重点**` 原样显示 → 纯文本), Tk 标签不支持 markdown。

## v5.0.2

- **截屏视觉自查修复 (权限打通后逐窗实拍)**:
  - 主窗「自动探查当前网络」按钮重复出现两次(档案类型行 + 表单字段区各一) → 删掉档案类型行的那个, 只留表单内带说明文字的一份。
  - 偏好设置网络报告提示文案过时: 「主界面宫格」→「主界面 功能导航」(v5 已无宫格)。
  - 视觉检查通过项: 主窗双栏/状态条三指标/按钮层级(Accent 保存 · Gray 检测 · Quiet 导入导出)正常; 隧道共享窗口(自检/两段方案/PAC/口令复选框/二维码)正常; 偏好设置(滚动条就位/勾选组/软件更新/通知/报告)正常, 全部无文字截断。

## v5.0.1

- **小屏适配大修 (13/14 寸 Mac 可视区 ~700px 场景)**:
  - 新增 `gui/scrollkit.py`: `fit_geometry()`(窗口打开高度钳制到屏幕可视区) + `make_scrollable()`(整页纵向滚动容器, 内容宽度自适应)。
  - **主窗口**: 整页装进滚动容器, 高度按 `min(880, 屏幕高-120)` 打开——矮窗口下所有功能滚轮可达, 不再被截在屏幕外; 右栏「偏好设置」等底部按钮滚动即达。排版密度整体压缩一档(约省 90px)。
  - **偏好设置**: 680×800 内容装进滚动容器(实测内容 1058px/视口 764px 滚动可达), 修一处 make_scrollable 返回的内框被重复 pack 导致滚动失效的 bug; 3 处 `wraplength=600` 超容器宽导致文字截断 → 540。
  - **全部子窗口**高度钳制: 隧道共享(760×780)/路由器中继(820×780)/路由器代理(880×820)/偏好(680×800)/向导(640×600)/帮助(560×560)/测速(680×570)/热点(720×620)/网络报告(660×560)/路由器检测(720×640)/VPN 对话框(460×300), 并下调各自 minsize 允许收缩。
  - 实测(模拟 1140×680 可视区): 主窗文字截断 0, 滚到底「偏好设置」可达; 偏好设置文字截断 0, 滚到底「保存设置」可达; 新增 `tests/test_small_screen_fit.py` (11 项), 全套 230 项通过。

## v5.0.0

- **主界面大改版: v2 双栏排版 + v4 全功能整合**:
  - 左栏「连接档案」内嵌主窗: 档案下拉/新建/删除 + 类型切换(校园网认证/普通WiFi) + 动态字段表单 + 自动探查当前网络 + 保存/立即检测/导入/导出, 一屏直达不再需要开子窗口。
  - 右栏「功能导航」按场景分组: **连接**(启动守护/立即检测/开机自启) · **共享上网**(隧道共享/热点分享/网络控制台) · **VPN 加速**(v5 新增) · **路由器**(中继/代理/检测) · **工具**(网络测速/新手向导/偏好设置)。
  - 底部运行日志恢复 v2 的收起/展开交互, 默认收起给表单让位; 展开时若屏幕有余量自动加高窗口, 不挤压表单。
  - 窗口默认 1140×880 (双栏布局加宽), 默认尺寸下全部文字完整显示, 无「文字只有一半」。
- **新增「VPN 加速」面板 (主窗直达)**:
  - 状态实时显示(未启用/已启用 host:port); 「配置 VPN 代理」弹窗填主机+端口(适配 Clash/机场等本机 HTTP 代理); 「一键填本机 7890」预设; 「停用」一键清除。
  - 启用后隧道共享的设备流量自动经 VPN 转发(复用既有 upstream_proxy 链路), 实现"电脑当网关+VPN 全透明"。
- **修复**: 连接档案独立窗口与主窗并存时 cmb_profile 等控件引用被争夺的问题——v5 起档案表单只存在于主窗, `open_profile_window` 保留为兼容入口(置前主窗)。
- 测试: 新增 `tests/test_v5_main_layout.py` (6 项); 更新 `test_main_grid_order.py` / `test_main_view_log_inline.py` / `test_typography_polish.py` / `test_router_proxy_window.py` 到 v5 结构; 全套 219 项通过。

## v4.0.5

- **修复「v4.0.4 默认开启防蹭网口令」导致手机 PAC/手动代理全部 407 被拒、上不了网**:
  - 现象: v4.0.4 起「隧道共享」每次启动都自动生成 `tunnel_shared_key` (共享口令) 并要求所有设备附带 `X-Shared-Key` HTTP 头, 但**iOS/Android 的 PAC/手动代理客户端根本无法附加自定义 HTTP 头**, 导致所有手机请求被代理以 `407 Proxy Authentication Required` 拒绝, 表现成「配了代理但完全没网」。
  - 根因: v4.0.4 引入的 `shared_key` 防蹭网特性默认开启且 UI 没暴露关闭入口, 普通用户(包括本次出问题的用户)根本没法让代理走通。
  - 修复 (3 处):
    1. `gui/tunnel_ui.py::toggle_share`: 默认不再生成口令 (`shared_key=None`)。只有当用户显式勾选 `cfg.tunnel_require_key=True` 时才生成。
    2. `gui/tunnel_ui.py::_show_tunnel_ready`: 在「② 电脑模式」卡里新增 **「启用口令保护」复选框**, 默认不勾。勾上时弹窗显示新生成的口令、提示需要在自定义调用里附 `X-Shared-Key: <key>`, 并热重启 SharedProxy 实例; 取消勾选时同样热重启到无口令模式。
    3. 配置持久化字段 `tunnel_require_key` 写入 `core/config.json`, v4.0.4 残留的口令 (有 `tunnel_shared_key` 但没 `tunnel_require_key`) 自动保持关闭状态, 不强制激活 (避免突然「被加密」)。
  - 新增 `tests/test_tunnel_default_no_key.py` (10 项回归): 默认无 key / 显式开 key / 5 种 v4.0.4 残留配置下的决策矩阵。
  - **操作步骤让手机立即能上网**:
    1. 更新到 v4.0.5 (一键更新或官网下载)。
    2. 重新点「隧道共享」开关一次 (旧实例仍在用 v4.0.4 的口令策略, 重启才能生效)。
    3. 手机 PAC 保持现状 (`http://10.52.188.32:8080/proxy.pac`) 即可——会自动走通。

## v4.0.4

- **修复偏好设置"立即更新"按钮不可见 (v4.0.4.1)**:
  - 现象: 偏好设置 → 软件更新 → 点"立即检查"查到新版本后, 文案写"点'立即更新'自动下载"但**找不到"立即更新"按钮**。
  - 根因: `_render_pref_update_result` 创建立即更新按钮时 `parent=self` (App 根窗口), `pack` 到根窗口被埋在底层不可见。
  - 修复: 保存 `_pref_card` 引用 (偏好设置的 card Frame), 按钮 parent 改用 `_pref_card`。
  - 体验优化: **"立即检查"按钮自动变身"立即更新"**——查到新版本时该按钮文字直接换成"立即更新" + command 换成 `_do_update`, 无需找按钮一键触达。
  - 新增 `tests/test_pref_update_button.py` (3 项回归)。

- **修正 keepalive_core.APP_VERSION 4.0.3 → 4.0.4 (v4.0.4.2)**:
  - 现象: 打包后的 .app 窗口标题栏写 "v4.0.3" 但代码已是 v4.0.4 (宫格 10 项、立即更新按钮等都对了)。
  - 根因: PyInstaller 打包只改了 Info.plist 的版本号, 源码 `keepalive_core.APP_VERSION` 还是字符串 "4.0.3"。
  - 修复: 同步源码版本号到 4.0.4, 重新打包。

- **修复 iOS 描述文件只配 HTTP 不配 HTTPS 导致手机上不了网**:
  - 现象: iPhone 扫码安装 mobileconfig 后, 浏览器仍上不了网 (HTTPS 站点全部超时, 表现成"配了代理但没生效").
  - 根因: `shared_proxy._ios_mobileconfig` 只填了 `HTTPEnable/HTTPPort/HTTPProxy`, iPhone Safari 默认 HTTPS 直连, HTTP 代理服务器收不到 CONNECT 隧道请求。
  - 修复: 同时填 `HTTPSEnable/HTTPSPort/HTTPSProxy` 三件套, iPhone Safari 自动 CONNECT HTTPS 到电脑 8080, 电脑代理转发到外网。
  - 新增 `tests/test_mobileconfig_https.py` (4 项回归): HTTP 三件套 + HTTPS 三件套 + 顶层 ProxyServer/Port + plist 合法 XML。

- **新增「路由器代理」独立窗口 + 主入口 (`gui/router_proxy.py` + 宫格 `路由器代理`)**:
  - 思路: 路由器直连校园网并完成认证 → 路由器自身开 HTTP 代理 (默认 8080) → 手机连路由器 WiFi 但不登录校园网 → 手动代理到「路由器 IP:8080」即上网。比「路由器中继 (WISP)」占用同一账号名额但更稳定 (路由器始终在线, 电脑关掉也不影响)。
  - 窗口四段:
    - **A. 路由器识别 + 适配固件查询**: 复用 `core.router_fingerprint()` + `lookup_firmware_urls()`, 一键探测品牌/型号/硬件版本, 显示厂商官网 + OpenWrt ToH + Merlin (华硕) 入口。
    - **B. 探测路由器 8080 端口**: 新增 `probe_router_proxy()` 函数, 通过 HTTP HEAD 探测常见端口 8080/8888/1080/3128/9090, 识别 `Proxy-Authenticate`/`Via`/`Server: squid|privoxy|tinyproxy|mikrotik|padavan|merlin|openwrt` 等代理特征。生成 PAC (data URL 形式, 路由器无需托管 PAC 文件) + 手动代理配置 + 设置页 URL + 二维码。
    - **C. 在路由器上启动 HTTP 代理 (按固件分步)**: OpenWrt (tinyproxy / luci-app-squid) / Padavan (老毛子内置 HTTP 代理) / Merlin (华硕 entware + JFFS) / iKuaiOS (爱快) / 其它 (OpenWrt ToH 适配表 + 刷机风险提示) 五个 Tab。
    - **D. 手机端配置步骤**: 按当前 IP/端口实时生成 iOS (WiFi 高级 → 配置代理 → 手动) + Android (WLAN 高级 → 代理 → 手动 / 自动) 详细步骤, 含 HTTPS CONNECT 兼容性说明。
  - 主界面: `_build_feature_grid` 升级 9 → 10 项, 第 2 行从 3 列扩展为 4 列, "路由器三件套" 集中展示: 路由器中继 / **路由器代理** / 路由器检测 / 网络控制台。
  - `App` 类继承 `RouterProxyMixin` (新增在 `gui/router_proxy.py`), 主入口走 `_fwin_open_legacy("router_proxy", self.show_router_proxy_window)` 单例。
  - 复用 `core.detect_gateway_mode()` 自动填路由器 IP, 用户可手动覆盖。

- **隧道共享窗口集成第三种上游模式 (③ 路由器代理)**:
  - `_show_tunnel_ready` 在 ① 路由器中继 + ② 电脑直连 之外, 新增 **③ 路由器代理段**: 路由器自身开 HTTP 代理, 手机走代理到路由器 IP:8080 (路由器一次性认证, 电脑关掉也能用)。
  - 段内含: 路由器 IP + 端口输入 + "生成设置页 + 复制" 按钮 + "打开「路由器代理」独立窗口" 跳转 + 4 步快速上手 (刷固件 → 装代理插件 → 手机配代理 → 上网)。
  - 几何 760×780 → **760×820 滚动**, 三段都能完整展示。

- **测试与回归**:
  - 新增 `tests/test_router_proxy_window.py` (5 项): mixin 存在 / 窗口四段齐全 (A 识别 + B 探测 + C OpenWrt/Padavan/Merlin/iKuaiOS + D iOS/Android) / `probe_router_proxy` 签名 / `build_router_pac` 格式 / `App` 继承 `RouterProxyMixin` / `_show_tunnel_ready` 含 ③ 路由器代理段。
  - 更新 `tests/test_main_grid_order.py`: 宫格 9 → 10 项, 第 2 行扩展为 4 列 (路由器中继 / 路由器代理 / 路由器检测 / 网络控制台)。
  - **累计 199 项测试全部通过**。

- **脚本登记**: `scripts/split_gui.py` 加注释说明 `gui/router_proxy.py` 不在 GROUPS 内 (一次性迁移工具不再处理新增 Mixin)。

## v4.0.3

- **隧道共享窗口集成两种上游模式（路由器中继 + 电脑直连）**：
  - 原 _show_tunnel_ready 把路由器/电脑模式只在状态文本里各提一句，重写为**滚动布局 + 两段独立卡片**：
    - **① 路由器中继（手机连路由器 WiFi）** — 路由器模式下强调「不用配任何代理」；电脑模式下提示「直接连路由器 WiFi 即可，无需本软件隧道」「若手机不在路由器覆盖范围 → 向下滚到 ② 段」
    - **② 电脑直连校园网（手机配代理）** — PAC + 手动代理 + IP 切换 + 二维码 + 4 步教程（PAC/手动/iOS/Android/首次授权/校园网覆盖范围内上网）
  - 顶部状态栏根据实际模式给精确文案（不模式时显示「未识别，下面两条独立方案按你实际情况选用」），保证任何上游情况都能用。
  - 几何 640×600 → **760×780 滚动**，方便内容超出屏幕。
  - 新增 tests/test_tunnel_mode_split.py (3 项回归) — 路由器模式 + 电脑模式文案分支 + detect_gateway_mode 不抛错。累计 191 项测试全绿。

- **网络控制台鉴权修复（解决手机浏览器刷新报"未接入互联网"）**：
  - 现象：手机连校园网不认证扫码访问控制台，第一次能打开、刷新就 403。iOS Safari 对 403 会展示并缓存"未接入互联网"误导文案，需重启 WiFi 才能恢复。
  - 修复：所有鉴权失败（GET / POST / API）改返 200 `KEY_ENTRY_HTML`（口令输入引导页），不再 403。
  - 响应头加固 `Cache-Control: no-store, no-cache, must-revalidate, max-age=0` + `Pragma: no-cache` + `Connection: close`，防止 Safari 缓存失败状态、防止后台连接断开被误判断网。
  - `/api/key` 旁路鉴权接口：永远返 JSON `{"authed": true|false}`，供引导页实时校验，避免跳到控制台后所有 API 返 HTML 导致页面卡死。
  - 引导页 enter() 先 fetch `/api/key` 校验，成功再跳转；失败提示"口令错误, 请重新复制"。
  - 引导页底部加 iOS Safari 缓存旁路提示文案。
- v4.0.2 同步：档案窗口按类型动态表单 / 自动探查当前网络 / 新建默认 wifi / save_profile 按类型校验。
- 新增 tests/test_web_console_auth.py（7 项鉴权回归）+ tests/test_shared_proxy_loopback.py（5 项 loopback URL 回归）；修整 tests/test_web_console.py 的 3 项旧 403 期望。累计 137 项测试全部通过。
- **共享代理识别"代理自己"的绝对 URL（解决 iPhone Safari 报未接入）**：
  - 现象：iPhone 通过 HTTP 代理访问 `http://192.168.3.1/`（电脑 bridge0 NAT 网关）或扫码 `http://10.52.188.32:8081/?key=...`（电脑 en0 IP 控制台）时，shared_proxy 把请求当外网转发到 host 自身 `:80`/`:8081`，返回 502 触发 Safari "未接入互联网"误导。
  - 修复：shared_proxy 启动时采集本机所有 IPv4 接口（en0/bridge0/bridge100/127.0.0.1 等）。收到 GET 绝对 URL 时若 host 是自己，分三路处理：
    1. host 是自己 + port 缺省/等于代理端口 → 视同相对路径走引导页/mobileconfig/PAC（避免 502）
    2. host 是自己 + port 等于本地服务端口（如 8081 控制台）→ 代理到 `127.0.0.1:port`（手机能正常拿到控制台）
    3. host 是自己 + 其他 port → 同样代理到 `127.0.0.1:port`（本地服务透传）

- **热点窗口：macOS 文案诚实化 + Windows 一键开热点 + 跨平台设备列表/流量**：
  - **macOS 文案**：明确写"不支持 Wi-Fi→Wi-Fi 共享（系统级硬限制，App 无法绕过）"，给出"USB 线连手机 / 改用隧道共享"两条替代方案，不再夸大功能。
  - **Windows 一键开启**：`start_mobile_hotspot()` 优先 PowerShell `Start-MobileHotspot`，netsh `wlan start hostednetwork` 兜底，返 `(ok, msg)` 给 UI。失败 fallback 跳系统设置页。
  - **设备列表 + 流量**：`list_hotspot_clients()` 跨平台识别 NAT 网关接口（macOS bridge0/bridge100；Windows 192.168.x/24 热点接口），用 arp 表 + OUI 拿客户端 IP/MAC/厂家。
  - **macOS 流量**：按 IP 拆分需 sudo（系统限制），改为显示 bridge0 接口总 RX/TX 字节 + 标注"粗略"。
  - **Windows 流量**：用 PowerShell `Get-NetIPStatistics` 精确按 IP 拿上传下载字节。
  - 热点窗口 Treeview 表格展示：IP / MAC / 厂家 / 接收 / 发送 + 底部说明提示。
  - 新增 `fmt_bytes()` 字节数 -> 人类可读 (B/KiB/MiB/GiB/TiB)。
  - 新增 tests/test_hotspot_clients.py（13 项回归）。累计 150 项测试全部通过 + 热点窗口 Tk 冒烟通过。
- **iPhone 配套说明**：HTTP 代理 server IP 应改为电脑当前 en0 局域网 IP（10.52.188.32），而不是 bridge0 的 NAT 网关（192.168.3.1）；后者只在 iPhone 连电脑热点子网时才可达。
- **「软件更新」融入偏好设置**：主界面宫格删除"软件更新"独立入口（偏好设置卡片说明加"自动更新"）。偏好设置里新增 inline 段：当前版本 + "立即检查"按钮 + 检查中/已是最新/发现新版本三态内联 + "立即更新"按钮（点击触发下载+自替换，不再弹模态窗口）。复用 `_bg_check_update(manual=True, on_result=on_result)` / `_do_update` 已有链路，仅换 UI 容器。
- **「网络报告」融入偏好设置**：主界面宫格删除"网络报告"独立入口（偏好设置卡片说明加"报告"）。偏好设置底部新增"网络报告"段：7 天稳定性摘要（summary + 5 项统计）+ 断网时间线（含持续时长）+ 「刷新报告」「导出诊断」两个按钮。`history_enabled=False` 时整段自动灰显 + 提示文案，引导用户去上方勾选保存历史。开启/取消勾选历史时实时同步报告段状态；切换保存路径后报告同步跟随 `cfg["history_log_path"]`。
- **「运行日志」回到主界面底部**：删 `open_log_window` 独立窗口。`app_gui._build_ui` 在功能宫格下方直接构造 `Card.TFrame` 包裹 `Text` + `Scrollbar`（深色等宽字体 + 滚动条 + 鼠标滚轮绑定），启动即调 `_load_existing_log` 灌入最近 50 条历史，`_poll_log` 持续实时滚动。功能宫格从 12 缩为 8 项（4×3 后两行拆除）。
- **「导出诊断」「使用帮助」融入偏好设置**：主界面宫格再删两个。偏好设置底部新增"诊断与帮助"段：「导出诊断」按钮（复用 `export_diag`，弹保存对话框 + 复制到剪贴板）+ 「使用帮助」按钮（复用 `show_help` 弹出原弹窗 — 内容多、需要滚动）。偏好设置卡片描述收敛为 8 项。
- 新增 tests/test_main_view_log_inline.py（3 项回归：open_log_window 已删/宫格不再引用旧入口/show_help 实现存在）。累计 167 项测试全部通过 + 主界面/偏好设置窗口 Tk 冒烟通过。
- **整体排版升级 PASS**：4 个窗体按 8px 网格统一几何 + 内边距 + 字段间距，主窗更舒展、子窗更宽松、字段对齐。
  - theme.py 新增排版系统常量: `PAD_XS/S/M/L/XL/XXL = 4/8/12/16/20/24`, `CARD_PAD_MAIN=(12, 10)` `CARD_PAD_SUB=(24, 22)` `WINDOW_PAD=(18, 18)`, `GAP_SECTION=20`, 各种 `DESC_WRAP_*` + `FONT_MONO`。
  - 主窗 1060x760 → 1080x820, minsize 920x700 → 960x740。顶栏内边距 14→18, 状态条 14/8→20/14, 守护控制 14/10→18/14, 底部日志 14/10→18/14 + 内边距 8/6 + height 8→10, 卡片 10/9→14/12, 卡片描述 wraplength 250→220 让 3 列下换行更整齐, 状态点 font 11→12。
  - 档案窗 720x560 → 780x620 (minsize 640x520→700x560), 卡片 18/14→24/22, 字段之间 column padx 12→16, 上下间隔 pady 4/2 → 10/4, 按钮间距 6→8-10。
  - 热点窗 640x560 → 720x620, 卡片 22/18→24/22, wraplength 580→640, 表格列宽 80→90-100。
  - 偏好设置 620x740 → 680x800 (minsize 580x600→640x660), 卡片 22/18→26/24, 段间距 14→20, 小间距 4→8, wraplength 460/560→600。
  - 所有按钮横向间距统一 GAP_BUTTON_X=8px，按钮区域上下间距 12-20px。
- 新增 tests/test_typography_polish.py（7 项回归：theme 常量存在 / 主窗/偏好/档案/热点几何 + 主界面 inline 日志）。累计 174 项测试全部通过。
- **「路由器方案」合并检测 + 中继 + 固件适配查询（主界面宫格改名 + 窗口重写）**：
  - 主界面「路由器检测」宫格 → 「路由器方案」，描述："检测 + 中继校园网步骤 + 官方固件查询"。
  - 窗口重写为三段式滚动布局: **A. 路由器识别**（品牌/型号/硬件版本/网关/MAC/管理页入口，保留原有保存/打开管理页按钮）+ **B. 路由器中继校园网方案**（内嵌 `core.router_guide()` + 自定义上游 SSID / 是否需认证 → 自动生成分步路径：华为/小米/TP-LINK/水星/迅捷/腾达/华硕/网件/中兴/360/D-Link/斐讯 各自品牌路径 + 通用模板）+ **C. 固件统一准备**（按品牌列 OpenWrt 适配入口 + 厂商官网 + 华硕 Merlin，按「查询官方适配」按钮一键生成）。
  - **核心 `lookup_firmware_urls(brand, model, revision)`**: 内置 12 品牌厂商查询 + OpenWrt ToH 直链，未识别品牌 fallback 到 openwrt.org/toh/start。
  - **核心 `download_firmware(url, save_path, expected_sha256, progress_cb)`**: 后台下载 + 进度回调 + 校验和（自动删除不完整文件）。这是关键边界 —— **本软件仅下载 + 校验，不刷入路由器**；用户在路由器 Web 管理页「系统升级」手动选文件升级。
  - **核心 `sha256_of_file(path)`**: 计算文件 SHA256，用于刷机前比对官方发布值，确保包完整/未被篡改。
  - 「下载到本地（不刷入）」按钮: 弹 URL + SHA256 输入对话框 + 选择保存位置 + 后台下载 + 进度条 + 校验失败自动回滚 + 完成后状态显示 SHA256。
- **主界面宫格重排（9 项 4 个路由器方案拆分）**：
  - 顺序按用户指定：连接档案 → 隧道共享 → 热点分享 → **路由器中继**(新独立宫格) → 路由器检测 → 网络控制台 → 网络测速 → 新手向导 → 偏好设置。
  - **「路由器中继」(新独立宫格)**：含 router_guide 中继方案 + 固件查询/下载到本地/B 段，与"路由器检测"分开。原来混在「路由器方案」窗口里的 B/C 段全部搬到 `show_router_relay_window`。
  - **「路由器检测」(精简)**：A 段保留（品牌/型号/硬件版本/网关/MAC/管理入口 + 检测/保存/打开管理页），底部按钮新增"去路由器中继方案"快捷跳转。
  - 路由器中继窗口独立 (`router_relay` 单例)，路由器检测窗口仍走 `router` 单例，不互相干扰。
- **未做（说明）**：「透明代理（系统级 TPROXY）」当前不在本软件内做（需要 tun 设备 + sudo + 跨平台 pf/iptables 差异大）；当前「电脑当网关 + VPN 全透明」用 **隧道共享窗口的「VPN 上游」按钮**实现 HTTP 流量转发（已存在，文档化在隧道窗内）。
- 新增 tests/test_router_firmware_lookup.py（10 项回归：12 品牌覆盖 + 未识别 fallback + sha256 + 文件下载 + 进度回调 + SHA256 校验失败回滚 + 异常 URL + router_guide 不抛错）。累计 184 项测试全部通过 + 路由方案窗口 Tk 冒烟通过。
- **稳定性历史保存地址可选**：`core.history.effective_log_path(cfg)` + `set_log_path()` + 模块级 reader 切换。`record_network_history` / `summarize_network_history` / `analyze_outage_timeline` 三处 reader 都跟随用户指定路径；启动时 `_restore_preferences` 调 `_sync_history_log_path()` 同步。`cfg["history_log_path"]` 默认空（走 `common.HISTORY_PATH`）。
  - **偏好设置里勾选行为**：勾选「保存网络稳定性历史」立即弹保存对话框（默认 `~/Downloads/campus_net_history.jsonl`，类型 `*.jsonl/*.txt/*.*`），用户取消则回滚勾选。已勾选时显示当前路径 + "更改"按钮，可随时重新指定。save_preferences 落盘 + 同步 history 模块 + 立刻写一条 "online" 探针事件，让文件可被读到。
  - 新增 tests/test_preferences_history_path.py（10 项回归）+ tests/test_preferences_report_inline.py（4 项网络报告渲染回归）。累计 164 项测试全部通过 + 偏好设置窗口 Tk 冒烟通过。

## v4.0.2

- **档案窗口按类型动态表单**：
  - 普通WiFi/热点档案：仅显示档案名 + 绑定WiFi/网关 + 检测间隔（不再变灰运营商/账号/密码/认证服务器——这些字段根本不存在）。
  - 校园网档案：显示完整字段（运营商/账号/密码/绑定/间隔/认证服务器）。
  - 切换档案类型立即重建字段区（不再留变灰禁用的废控件）。
  - **新增「自动探查当前网络」按钮**：一键填入电脑当前 SSID 与 网关（家庭/中继/热点场景秒建档案）。
  - 新建档案默认类型改为「普通WiFi/热点」（更贴近新用户"只想检测断网"的需求）。
- save_profile 按类型校验：wifi 档案必须绑定 SSID 或 网关之一，不写 username/password/auth_url/login_type（避免空字段堆积）；campus 档案必须有账号密码与认证服务器。
- 新增 tests/test_profile_form_types.py（2 项回归）；累计 124 项测试全部通过 + 动态表单 Tk 冒烟通过。

## v4.0.1

- **修复隧道共享二维码/地址给错 IP（重要）**：多网卡电脑（连校园网 + Clash TUN + 历史热点/互联网共享接口）下，`get_lan_ips()` 用 `getaddrinfo` 取 IP 顺序不稳定，可能把虚拟网卡地址（如 192.168.3.1）排在校园网出口前，手机扫到的二维码/代理地址根本不可达。
  - 重写 IP 选取：解析 `ifconfig`，丢弃纯虚拟接口（utun/tun/ppp/awdl 等）与 Clash 假 IP（198.18.0.0/15）；保留真实网卡与 bridge（热点场景）；**与默认出口网关同网段的 IP 排最前**（默认路由被 VPN 抢走时回退 `netstat -rn` 找真实网关）。
  - 隧道就绪窗口新增「地址切换」下拉：校园网直连 / 电脑热点等不同接入方式可手动换服务器 IP（同步更新 PAC/二维码/手动代理提示）。
  - 修正引导文案：不再提示"代理头带口令"（实际为新设备首次访问由电脑弹窗授权）。
- 新增 tests/test_lan_ips.py（3 项多网卡排序回归）；累计 122 项测试全部通过。

## v4.0.0

- **界面大改版「独立窗口版」**：每个功能一个独立窗口，不再把档案表单/工具按钮/日志挤在主界面。
  - 主窗口瘦身为「总览 + 功能宫格」：顶部状态条（守护/网络/环境）+ 守护开关（启动/停止 + 开机自启 + 立即检测）+ 13 个功能卡片。
  - 新增独立窗口：连接档案管理（完整编辑表单独立成窗）、运行日志（实时滚动的独立窗口）、热点分享（状态检测+一键打开系统热点设置）、网络报告（7 天概况+断网时间线独立成窗）、软件更新（检查更新/立即更新独立成窗）。
  - 偏好设置拆纯设置（网络历史/合盖保活/防踢/通知），不再内嵌报告与更新。
  - 所有功能窗口单实例：重复点击卡片只把已开窗口置前，不产生堆叠窗口。
  - 隧道共享就绪窗口新增「停止共享」按钮，可在一个窗口内完成 开启→配置→停止 全流程。
  - 档案切换/守护在档案窗口未打开时不再依赖界面控件（_current_profile 回退 cfg.active_profile）。
- 新增 gui/feature_windows.py（FeatureWindowsMixin 窗口注册表 + 各窗口构建器）；119 项测试全部通过 + 全窗口 Tk 冒烟通过。

## v3.1.8

- 偏好设置窗口适配新「软件更新」面板：高度 640→700 + 允许纵向拉伸，避免状态行被裁切。

## v3.1.7

- 偏好设置「软件更新」面板布局修正：原布局把「当前版本」与「发现新版本」状态文字同挤一行，导致文字重叠。现拆为两行——版本独占一行，状态行只放结果 + 操作按钮，文字不再重叠。同时「已是最新」时收起立即更新按钮，状态语义自洽。
- PyInstaller 打包增加 `--collect-all certifi`，Windows 端更新检查同样带上证书包（macOS 已在 v3.1.5 通过 certifi 兜底验证）。

## v3.1.6

- 自动更新端到端验证（v3.1.5 实测）：打包 App 的启动自动检测已能在日志输出"已是最新/发现新版本"，本版用于在真机验证"发现新版本→弹窗→立即更新→自替换重启"完整链路。
- certifi 证书兜底同时覆盖 Windows 打包版（requirements 同步生效）。

## v3.1.5

- 【重要修复】更新检查在打包后的 App 里一直静默失败的真因：PyInstaller 冻结包找不到系统 CA 证书链，连 GitHub 报 `CERTIFICATE_VERIFY_FAILED`，导致"打开软件自动检查更新→有新版本弹窗"从未真正触发。修复：内置 certifi 证书包兜底（requirements 增加 certifi，updater 用 `_ssl_context()` 加载）。此版起"启动自动检测+非最新弹窗"真正可用。

## v3.1.4

- 偏好设置新增「软件更新」面板（内联，不依赖弹窗）：显示当前版本 + 「检查更新」按钮，点击后立即联网检测并在面板内显示结果——「✓ 已是最新」或「发现新版本 vX.Y.Z」+ 内联「立即更新」按钮（一键下载自替换）。此面板在主窗口可见性受限的环境下也能可靠完成"点击→检测→获取最新版"。
- 更新检查链路支持 on_result 回调（内联 UI 使用）。

## v3.1.3

- 更新链路诊断埋点：检查失败/已最新/发现新版本/弹窗创建成功与否都会写入运行日志（此前更新弹窗在某些环境静默不出现，无法定位）。
- 弹窗前确保主窗口可见（若在托盘/后台先恢复显示），再创建对话框，避免窗口藏于托盘导致漏看。

## v3.1.2

- 更新弹窗置前（deiconify/lift/topmost）：App 在托盘或后台时，发现新版本的窗口不再可能被漏看。
- CI 发布包：macOS zip 内 .app 改 ASCII 名 CampusNetManager.app（中文条目名在部分解压器如 Python zipfile 中会乱码），资产与包内名统一 ASCII 更稳。
- 自动更新真实链路已在 3.1.0 → 3.1.1 走通（检测/下载/解压/替换脚本/去隔离/重开）；本版起更新弹窗不会再藏。

## v3.1.1

- 修复档案匹配设计缺陷（2026-09-06 手机热点事故根因）：用户显式选中的可登录校园网档案，在无 SSID/网关精确匹配时优先于「空绑定默认档案」被采用（此前会被空绑定档案抢走去登录，导致用错账号）；守护自动切换增加守卫——活跃档案可登录且校园网可达时，不再因"认证可用"自动切去别的默认档案，仅 SSID/网关精确匹配才切换。
- 新增 test_profile_preference.py 回归测试 7 项（含事故场景还原）；累计 119 项测试全部通过。
- Web 控制台增强：状态页显示电脑名称与本机局域网 IP（手机连控制台即可判断连的是哪台电脑）。
- 配置默认值 ensure_preferences 补全逻辑整理。

## v3.1.0

- 自动更新：启动时后台检查 GitHub Release（20 小时节流、可跳过版本、偏好设置可关）；发现新版本弹窗展示更新说明，一键下载后自替换重启（macOS bash / Windows bat 等待退出后替换）；下载失败可转发布页手动下载。偏好设置新增「检查更新」按钮。
- 局域网 Web 控制台：主界面「网络控制台」一键开启（端口 8081，口令鉴权，地址+二维码直达）。手机/电脑浏览器可查看守护/网络/环境状态、最近日志、断网时间线、隧道已授权设备（可移除），并可远程启动/停止守护。
- 新增 updater.py（16 项测试）与 web_console.py（12 项测试）；gui 新增 update_ui / console_ui 两个 Mixin。
- 偏好修正：ensure_preferences 现在会补全 kick_guard 与自动更新相关缺省键（旧配置升级兼容）。
- 112 项测试全部通过 + Tk 实例化冒烟通过。

## v3.0.0

- 代码拆分（可维护性大版本）：
  - `keepalive_core.py`（1970 行单文件）拆为 `core/` 包：common(常量) / config(档案配置) / history(网络历史) / netinfo(网络信息) / speed(测速) / router(路由器) / portal(认证探测) / matching(档案匹配) / auth(Dr.COM认证) / sysutils(日志通知自启) / daemon(守护线程)。`keepalive_core.py` 保留为门面（re-export + APP_VERSION 单源 + 诊断/实例锁），旧 import 与 patch 路径兼容。
  - `app_gui.py`（1987 行单类）拆为 `gui/` Mixin：theme(主题) + profile_form / router_tools / speed_window / tunnel_ui / preferences / tray / daemon_ctl / wizard 八个 Mixin，App 继承组装；主题常量独立 `gui/theme.py`。
  - 跨模块调用统一 `模块.名字` 风格——测试 patch 永远指向定义所在模块，杜绝签名/归属漂移。
  - 拆分脚本留档 `scripts/split_core.py`、`scripts/split_gui.py`。
- 测试 patch 目标全部迁移到子模块命名空间；84 项测试全绿 + Tk 实例化冒烟通过。
- 构建打包显式收集 core/gui 子模块（build_macos.sh 与 CI Windows 构建同步更新）。

## v2.9.6

- 守护异常兜底加固：连续异常现在计数并记录完整堆栈，第 5 次起退避延长到 60 秒并发送系统通知（不再无声空转）；健康循环自动清零。历史教训：回调签名失配曾被兜底静默吞掉导致死循环。
- 界面回调隔离：on_status / on_env / on_log 回调抛错单独记录「界面回调异常」，不再误计入守护异常，也不会拖死守护循环。
- 新增 test_daemon_resilience.py（4 项）：连续异常告警/退避、健康清零、回调隔离、on_status 5 参签名防回归。
- 修复 test_router_and_speed 卡死：on_status 回调已扩展为 5 参，测试 lambda 同步更新。
- 版本号单源化：新增 scripts/sync_version.py，从 APP_VERSION 同步 README 徽章并校验 CHANGELOG（--check 模式供 CI 使用）。
- 新增 GitHub Actions CI：push 自动在 macOS/Windows 双平台跑全部测试 + 版本一致性校验；打 v* 标签自动构建 macOS zip 和 Windows exe 并附加到 Release。
- 移除过时的 校园网连接管家.spec（含硬编码旧路径，构建脚本并不使用它）。
- 84 项测试全部通过。

## v2.9.5

- 修复「档案类型」下拉框与「连接档案」下拉框重合：类型行下移，grid 布局行号整体顺延。

## v2.9.4

- 新增「档案类型」：校园网认证（登录保活）/ 普通WiFi·热点（只检测断网）。新建热点、家里WiFi档案时选「普通WiFi」，认证服务器自动留空置灰，不再误填校园网地址。
- 普通WiFi·热点档案的守护逻辑：不登录校园网，但持续检测网络连通性，**断网时通知用户**（如手机热点断开、家里断网）。
- 新增 profile_is_wifi() 判断档案类型；旧档案兼容（有账号+认证服务器→视为校园网；空档案→视为普通WiFi）。
- 智能识别：用户选的档案类型决定守护是"登录保活"还是"只检测断网通知"。

## v2.9.3

- 智能档案自动切换：守护识别到环境（校园网/家庭路由器）后，若用户当前选的档案不匹配，自动切换到匹配的档案——① 有线接路由器中继校园网（无SSID，认证可达）→ 自动切到校园网档案；② 连家庭路由器（SSID匹配）→ 自动切到对应档案；③ 在家WiFi认证不可达 → 不误切。防止用户误选「任意网络」或选错档案时守护休眠不工作。
- 新增 best_match_profile()：SSID精确 > 网关精确 > 认证可达的校园网档案，三级匹配。

## v2.9.2

- 副路由器中继扩展信号：新手向导「方式 B」升级，引导副路由器中继连接（按品牌精确到菜单），并支持两种上游目标——① 校园网(学校场景) ② 主路由器 WiFi(家庭场景,主路由接光猫)。
- 中继指引参数化：`_brand_relay_guide(brand, target_ssid, need_auth)` 替代硬编码，副路由中继连主路由 WiFi 也能给出正确指引（不再写死校园网）。

## v2.9.1

- 「任意网络使用」绝不登录：用户选任意网络档案时，无论认证服务器探测结果如何，守护一律视为非校园网直接休眠，不再尝试登录（在家 WiFi 等场景不会误登录）。
- 状态栏显示修正：任意网络+非校园网时，「网络」栏显示绿色"WiFi 连接正常"（不再显示"VPN在线（校园认证掉线）"黄点）；「环境」栏显示"非校园网环境 守护休眠"。
- 认证服务器不硬填：任意网络档案不再预填学校认证服务器地址，留空可探测/自行填写。
- 光猫管理页探测增强：支持 80/8080 端口与光猫识别词（GPON/光猫/烽火等），并给出光猫常见管理地址指引。

## v2.9.0

- 电脑当网关 + VPN 全透明：隧道共享支持「VPN 上游代理」——接入隧道的设备流量经你的 VPN 客户端转发（HTTP CONNECT 隧道注入），实现设备零配置走 VPN。
- 新增「VPN 上游」配置入口（隧道共享就绪窗口），填本机 VPN 客户端代理端口（如 Clash 127.0.0.1:7890）即可；留空则设备直连外网。
- VPN 上游 CONNECT 端到端验证通过：客户端→SharedProxy→上游 VPN 隧道打通。
- 75 项测试全部通过。

## v2.8.0

- 扫码自动配置（三平台）：统一智能引导页自动识别设备系统（iOS 描述文件一键安装 / 安卓鸿蒙一键填充），手机扫二维码后按系统提示点一次即配好，不用手动输代理地址。
- iOS 配置描述文件：新增 /setup.mobileconfig 端点，iPhone/iPad 扫码下载后点「安装」自动配置代理。
- 网关模式自动识别：隧道共享界面显示当前是「电脑直连」还是「经路由器接入」，并给出对应连接建议；路由器模式提示手机直接连路由器 WiFi 即可上网。
- 一键部署：新增「🚀 一键部署」按钮，自动开启隧道 + 开机自启（隧道长期在线），日常主力使用零门槛。
- 72 项测试全部通过。

## v2.7.2

- 修复隧道共享引导弹窗字体截断：窗口调高，引导文案精简为短句，完整显示账号/口令。
- 优化手机扫描引导页排版：适配窄屏，代理地址更清晰。

## v2.7.1

- 修复偏好设置通知分类字体截断：通知开关改为单列纵向排列，长文案不再被窗口宽度截断。
- 优化热点设置引导：macOS 打开「系统设置→通用→共享」时给出更精简的分步操作指引（说明安全限制需手动确认一次）。

## v2.7.0

- 新增断网时间线：记录每次「何时断网 → 何时恢复 → 持续多久」，在偏好设置里展示最近 7 天的掉线记录，帮你判断路由器过热/链路问题的规律（新增 analyze_outage_timeline）。
- 新增中继伪装检测：路由器检测窗口现在能扫描局域网内可见设备，评估「多设备共享」泄露风险，并给出不掉速的轻量伪装建议（改 TTL 统一 / 关 WPS / 隐藏 SSID）。
- 隧道共享防蹭网：默认生成随机共享口令，借网设备需带口令才能上网，防止局域网内他人蹭网（新增 gen_tunnel_key + SharedProxy 口令校验）。
- 61 项测试全部通过。

## v2.6.3

- 修复连手机热点（无 SSID）时仍被硬拉去登录校园网：is_campus_locked 现在接受 respect_user_choice，用户在「任意网络使用」档案下即使无 SSID 也不再锁定为校园网，程序保持中立休眠，不再瞎重登。
- 54 项测试全部通过。

## v2.6.2

- 修复下拉框切换档案未生效：切换「任意网络使用」等档案时，现在会同步更新并保存 active_profile（之前只刷新表单显示，config 里的激活档案没变，导致守护仍用旧档案，环境栏显示错误）。
- 51 项测试全部通过。

## v2.6.1

- 尊重「任意网络使用」选择：用户在下拉框选用空账号的任意网络档案时，不再自动回退到立达校园网档案，环境栏如实显示所选档案，也不再强制显示校园网环境（新增 respect_user_choice 判定 + 5 项测试）。
- 中继掉线提示更准：自动重登失败时区分「认证服务器不可达（校园网链路断开，需重启路由器）」（可登录但账号问题）两种情况，网络管理员无需再困惑。
- 51 项测试全部通过。

## v2.6.0

- 环境判定覆盖有线中继：认证服务器短暂探测不到时，有线接路由器（无 SSID）环境也按校园网处理并尝试重登，不再误判「非校园网」后休眠（新增 is_campus_locked 判定 + 5 项测试）。
- 合盖保活与守护解耦：App 启动即恢复合盖/休眠保持运行偏好（不依赖守护是否启动）；停止守护不再误停用户已开启的合盖保活。
- 失败提示更具体：认证在线但出口不通 / 自动登录失败时，提示可能被其他设备占用名额、可在自助系统下线处理。

## v2.5.0

- 新增「防踢保活」：周期性刷新登录会话，让本机/路由器会话保持最新，名额满时第 3 台设备登录被挤掉的是别人而不是本设备（偏好设置可开关，默认开启）。
- 修复测速「VPN 对比失败」：VPN 对比测速两条路径独立容错，任一路径失败不再整体报错，成功路径照常显示结果。
- 实测确认中继场景会话归属（路由器 00:10:F3 占名额、登录按来源 IP 刷新续期），防踢与自动重登机制均验证可用。

## v2.4.0

- 新增「合盖/休眠时保持运行」：macOS 用 caffeinate 电源断言阻止系统/空闲睡眠，守护线程在合盖或系统空闲时继续联网保活，掉线后仍能自动重登（可在偏好设置中开关）。
- 修复中继场景「掉线后不动作」：绑定明确 SSID（如连中继路由器 WiFi / 直连 LIDA）时，即使认证服务器暂时探测不到，也按校园网环境处理并尝试重登，不再误判「非校园网」后静止休眠。
- 修复中继场景「重登失败」：档案匹配更智能，中继时优先选用已填写账号的校园网档案，避免落到空账号的默认档案导致登录失败。

## v2.3.3

- 自动登录成功后立即复检认证、VPN 和校园网直连路径。
- 自动同步刷新顶部网络状态、环境、匹配档案和检测时间。
- 修复慢速线路持续传输时因 25 秒上限被误报为测速失败的问题。
- 33 项自动化测试通过。

## v2.3.2

- 通知总开关关闭时，四类子通知立即取消勾选并禁用。
- 明确绿色对号表示已选择，点击“保存设置”后正式生效。

## v2.3.1

- macOS 复选框统一改为绿色底白色对号，未选中时显示空框。

## v2.3.0

- macOS 账号密码改存系统钥匙串，配置文件和安全导出不保存明文密码。
- 隧道共享新增启动自检、手机引导页、PAC 与二维码，保留新设备授权控制。
- 新增默认关闭的网络稳定性历史和面向普通用户的最近 7 天汇总。
- 新增系统通知总开关，以及掉线、恢复、恢复失败和新设备请求四类开关。

## v2.2.x

- 内置立达校园网专属档案。
- 新增多来源认证服务器探测，可识别 HTTP、HTML 与 JavaScript 跳转。
- VPN 状态与校园网直连状态分开判断，避免 VPN 场景误报假在线。
- 新增自动 VPN 对比测速、延迟、抖动、请求成功率和网络质量评分。
- 新增路由器只读体检、管理入口识别和安全刷机条件检查。
- 重做主界面固定布局，只有运行日志区域可独立滚动。
